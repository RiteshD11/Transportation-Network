...
const Graph = (() => {
  ...
  function updateEdge(edgeId, weights) {
    const edge = edges.find((e) => e.id === edgeId);
    if (!edge) throw new Error("Edge not found");

    const distance = parseFloat(weights.distance) || 0;
    const time = parseFloat(weights.time) || 0;
    const cost = parseFloat(weights.cost) || 0;
    const capacity = parseFloat(weights.capacity) || 100;
    const traffic = parseFloat(weights.traffic) || 0;

    if (distance <= 0 && time <= 0 && cost <= 0)
      throw new Error("At least one of Distance/Time/Cost must be positive.");

    edge.distance = distance;
    edge.time = time;
    edge.cost = cost;
    edge.capacity = capacity;
    edge.traffic = Math.max(0, Math.min(traffic, capacity)); // Ensure traffic doesn't exceed capacity
    edge.weight = distance; // backward compat
  }
  ...
  return {
    addNode, removeNode,
    addEdge, updateEdge, removeEdge, setEdgeTraffic,
    getNode, getEdge, getNodes, getEdges,
